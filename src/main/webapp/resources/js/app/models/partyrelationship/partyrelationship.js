...
define([ 
    ...
    var PartyRelationship = Backbone.Model.extend({
        ...
        validate: function (attrs) {
            var errors = [];
            if (!attrs.partyRole) {
            	errors.push({name: 'partyRole', message: entities_strings.alantra_form_field_required + entities_strings.partyrelationship_partyrole + '.'});
        	}	
            ...
            if (!attrs.toRoleId) {
            	errors.push({name: 'toRoleId', message: entities_strings.alantra_form_field_required + entities_strings.partyrelationship_toroleid + '.'});
        	}	
            if (!attrs.code) {
